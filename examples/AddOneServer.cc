...
class AddOneServer : noncopyable {
 public:
  AddOneServer(EventLoop* loop, const InetAddress& addr, size_t threadNums = 1,
               Nanoseconds timeout = 30s, size_t threadPoolSize = 8)
      : loop_(loop),
        server_(loop, addr),
    ...
    server_.setConnectionCallback(
        std::bind(&AddOneServer::onConnection, this, std::placeholders::_1));
    server_.setMessageCallback(std::bind(&AddOneServer::onMessage, this,
                                         std::placeholders::_1,
                                         std::placeholders::_2));
    server_.setWriteCompleteCallback(
        std::bind(&AddOneServer::onWriteComplete, this, std::placeholders::_1));
  }

  ~AddOneServer() { loop_->cancelTimer(timer_); }
  ...
  void onConnection(const TcpConnectionPtr& conn) {
    INFO("connection {} is {}", conn->name(),
         conn->connected() ? "up" : "down");

    if (conn->connected()) {
      conn->setHighWaterMarkCallback(
          std::bind(&AddOneServer::onHighWaterMark, this, std::placeholders::_1,
                    std::placeholders::_2),
          1024);
    } else
  ...
  void onMessage(const TcpConnectionPtr& conn, Buffer& buffer) {
    TRACE("connection {} recv {} byte(s)", conn->name(),
