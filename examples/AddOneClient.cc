...
class UserInput : noncopyable {
 public:
  UserInput(const TcpConnectionPtr& conn) : conn_(conn) {
    conn_->setMessageCallback(std::bind(&UserInput::onMessage, this,
                                        std::placeholders::_1,
                                        std::placeholders::_2));
  }
...
class AddOneBench : noncopyable {
 public:
  AddOneBench(EventLoop* loop, const InetAddress& addr)
      : loop_(loop), client_(loop, addr) {
    // 连接成功时 额外的回调
    client_.setConnectionCallback(
        std::bind(&AddOneBench::onConnection, this, std::placeholders::_1));
  }
  ...
  void onConnection(const TcpConnectionPtr& conn) {
    ...
    // 额外的线程   用于发送信息
    // 并利用userinput设置TcpConnection::messageCallback
    if (conn->connected()) {
