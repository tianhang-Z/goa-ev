...
class EchoServer : noncopyable {
 public:
  EchoServer(EventLoop* loop, const InetAddress& addr, size_t threadNums = 1,
             Nanoseconds timeout = 30s)
      : loop_(loop),
        server_(loop, addr),
  ...
  ~EchoServer() { loop_->cancelTimer(timer_); }
  ...
  void onConnection(const TcpConnectionPtr& conn) {
    ...
    if (conn->connected()) {
      conn->setHighWaterMarkCallback(
          std::bind(&EchoServer::onHighWaterMark, this, std::placeholders::_1,
                    std::placeholders::_2),
          1024);
