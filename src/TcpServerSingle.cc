...
void TcpServerSingle::setConnectionCallback(
...
void TcpServerSingle::setWriteCompleteCallback(
    const WriteCompleteCallback& callback) {
  writeCompleteCallback_ = callback;
}

void TcpServerSingle::start() { acceptor_.listen(); }

// 这里的逻辑将会传递给acceptor_.setNewConnectionCallback，
// 当acceptfd_有可读事件触发，即有新连接请求到来时，就执行该逻辑
void TcpServerSingle::newConnection(int connfd, const InetAddress& local,
                                    const InetAddress& peer) {
  loop_->assertInLoopThread();
  // 创建一个shared_ptr<TcpConnection>对象 赋给conn
  auto conn = std::make_shared<TcpConnection>(loop_, connfd, local, peer);
  connections_.insert(conn);
  conn->setMessageCallback(messageCallback_);
  conn->setWriteCompleteCallback(writeCompleteCallback_);
  conn->setCloseCallback(std::bind(&TcpServerSingle::closeConnection, this,
                                   std::placeholders::_1));
...
void TcpServerSingle::closeConnection(const TcpConnectionPtr& conn) {
  ...
  if (ret != 1) {
    FATAL(
        "TcpServerSingle::closeConnection connection set erase fatal, ret = {}",
        ret);
  }
  connectionCallback_(conn);
}