...
void TcpServer::setWriteCompleteCallback(
    const WriteCompleteCallback& callback) {
  writeCompleteCallback_ = callback;
}

void TcpServer::startInLoop() {
  INFO("TcpServer::start() {} with {} eventLoop thread(s)", local_.toIpPort(),
       numThreads_);

  baseServer_ = std::make_unique<TcpServerSingle>(baseLoop_, local_);
  /**
   * 如果想改主从Reactor结构，从这里入手，如果只有一个Reactor，则baseServer_回调即为外部传进来的回调，
   * 否则，baseServer_执行自己的回调（TcpServer中添加回调，来实现连接分配算法），
   * numThreads>1时，创建子Reactor，子Reactor执行外部传进来的回调。Connection对象绑定loop的步骤也需要修改位于TcpServerSingle.cc
   *: 32
   **/
...
void TcpServer::runInThread(size_t index) {
  EventLoop loop;
  TcpServerSingle server(&loop,
                         local_);  // 子EventLoop中的单独TcpServerSingle实例
