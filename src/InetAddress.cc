...
InetAddress::InetAddress(uint16_t port, bool loopback) {
  ...
  in_addr_t ip =
      loopback ? INADDR_LOOPBACK : INADDR_ANY;  //是否设为回环地址127.0.0.1
...
void InetAddress::setAddress(const sockaddr_in& addr) { addr_ = addr; }

const sockaddr* InetAddress::getSockaddr() const {
  return reinterpret_cast<const sockaddr*>(&addr_);
}

socklen_t InetAddress::getSocklen() const { return sizeof(addr_); }
