...
pid_t internalGettid() { return static_cast<pid_t>(syscall(SYS_gettid)); }
...
但程序仍然尝试向其发送数据，就会产生 SIGPIPE
信号。为了避免进程终止，可以通过忽略 SIGPIPE 信号来解决问题。
*/
class IgnoreSigPipe {
 public:
...
void EventLoop::runInLoop(const Task& task) {
  if (isInLoopThread()) {  //如果是初始线程 则执行任务
    ...
    queueInLoop(task);  //若不是 则任务加入队列 并唤醒初始线程
}
...
void EventLoop::assertNotInLoopThread() { assert(!isInLoopThread()); }
bool EventLoop::isInLoopThread() { return tid_ == internalGettid(); }
...
void EventLoop::handleRead() {
  ...
  if (n != sizeof(one)) {
    SYSERR("EventLoop::handleRead() should read() {} bytes, but {}",
           sizeof(one), n);
  }
}
