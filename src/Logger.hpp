...
std::vector<std::string_view> logLevelStr{"[ TRACE]", "[ DEBUG]", "[  INFO]",
                                          "[  WARN]", "[ ERROR]", "[ FATAL]"};
...
inline void logSys(const std::string& file, int line, int to_abort,
...
inline void logBase(const std::string& file, int line, LOG_LEVEL level,
                    int to_abort, const std::string& fmt, Args... args) {
...
#define TRACE(fmt, ...)                                                     \
  if (static_cast<unsigned>(goa::ev::logLevel) <=                           \
      static_cast<unsigned>(goa::ev::LOG_LEVEL::LOG_LEVEL_TRACE))           \
    goa::ev::internal::logBase(__FILE__, __LINE__,                          \
                               goa::ev::LOG_LEVEL::LOG_LEVEL_TRACE, 0, fmt, \
                               ##__VA_ARGS__);

#define DEBUG(fmt, ...)                                                     \
  if (static_cast<unsigned>(goa::ev::logLevel) <=                           \
      static_cast<unsigned>(goa::ev::LOG_LEVEL::LOG_LEVEL_DEBUG))           \
    goa::ev::internal::logBase(__FILE__, __LINE__,                          \
                               goa::ev::LOG_LEVEL::LOG_LEVEL_DEBUG, 0, fmt, \
                               ##__VA_ARGS__);

#define INFO(fmt, ...)                                                     \
  if (static_cast<unsigned>(goa::ev::logLevel) <=                          \
      static_cast<unsigned>(goa::ev::LOG_LEVEL::LOG_LEVEL_INFO))           \
    goa::ev::internal::logBase(__FILE__, __LINE__,                         \
                               goa::ev::LOG_LEVEL::LOG_LEVEL_INFO, 0, fmt, \
                               ##__VA_ARGS__);

#define WARN(fmt, ...)                                                     \
  if (static_cast<unsigned>(goa::ev::logLevel) <=                          \
      static_cast<unsigned>(goa::ev::LOG_LEVEL::LOG_LEVEL_WARN))           \
    goa::ev::internal::logBase(__FILE__, __LINE__,                         \
                               goa::ev::LOG_LEVEL::LOG_LEVEL_WARN, 0, fmt, \
                               ##__VA_ARGS__);

#define ERROR(fmt, ...)                                                     \
  if (static_cast<unsigned>(goa::ev::logLevel) <=                           \
      static_cast<unsigned>(goa::ev::LOG_LEVEL::LOG_LEVEL_ERROR))           \
    goa::ev::internal::logBase(__FILE__, __LINE__,                          \
                               goa::ev::LOG_LEVEL::LOG_LEVEL_ERROR, 0, fmt, \
                               ##__VA_ARGS__);

#define FATAL(fmt, ...)                                                     \
  if (static_cast<unsigned>(goa::ev::logLevel) <=                           \
      static_cast<unsigned>(goa::ev::LOG_LEVEL::LOG_LEVEL_FATAL))           \
    goa::ev::internal::logBase(__FILE__, __LINE__,                          \
                               goa::ev::LOG_LEVEL::LOG_LEVEL_FATAL, 1, fmt, \
                               ##__VA_ARGS__);

#define SYSERR(fmt, ...) \
  goa::ev::internal::logSys(__FILE__, __LINE__, 0, fmt, ##__VA_ARGS__);

#define SYSFATAL(fmt, ...) \
  goa::ev::internal::logSys(__FILE__, __LINE__, 1, fmt, ##__VA_ARGS__);
