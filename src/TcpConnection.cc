...
void defaultConnectionCallback(const TcpConnectionPtr& conn) {
  INFO("Connection {} -> {} {}", conn->peer().toIpPort(),
       conn->local().toIpPort(), conn->connected() ? "up" : "down");
}
void defaultMessageCallback(const TcpConnectionPtr& conn, Buffer& buffer) {
  TRACE("Connection {} -> {} recv {} bytes", conn->peer().toIpPort(),
        conn->local().toIpPort(), buffer.readableBytes());
  buffer.retrieveAll();
}
...
void TcpConnection::setHighWaterMarkCallback(
    const HighWaterMarkCallback& callback, size_t mark) {
  highWaterMark_ = mark;
...
void TcpConnection::connectEstablished() {
  ...
  state_ = kConnected;
  // 将socketfd_的Channel和TcpConnection绑定
  channel_.tie(shared_from_this());
  channel_.enableRead();  // 打开socket的读   并加入到epoll监听
}
...
const InetAddress& TcpConnection::local() const { return local_; }
const InetAddress& TcpConnection::peer() const { return peer_; }
...
bool TcpConnection::isReading() { return channel_.isReading(); }

const Buffer& TcpConnection::inputBuffer() const { return inputBuffer_; }
const Buffer& TcpConnection::outputBuffer() const { return outputBuffer_; }
...
void TcpConnection::handleWrite() {
  if (state_ == kDisconnected) {
    WARN("TcpConnection::handleWrite() disconnected, give up writing {} bytes",
  ...
  ssize_t n =
      ::write(sockfd_, outputBuffer_.peek(), outputBuffer_.readableBytes());
  if (n == -1) {
...
//调用send会将此任务放入eventloop的队列中，由eventloop的线程执行
void TcpConnection::sendInLoop(const char* data, size_t len) {
  ...
  //如果没有监听epollout事件 直接执行下面的逻辑 尝试直接对fd写数据
  ...
  if (!faultError && remain > 0) {
    if (highWaterMarkCallback_) {
      ...
      if (oldLen < highWaterMark_ && highWaterMark_ <= newLen)
        loop_->queueInLoop(
...
int TcpConnection::stateAtomicGetAndSet(int newState) {
  return __atomic_exchange_n(&state_, newState,
                             __ATOMIC_SEQ_CST);  // 顺序一致性内存序
}